...
import DataTableWithCRUD from '@/app/(full-page)/component/datableekstra/page';
...
const TambahSectionStory = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const response = await axios.get(API_ENDPOINTS.GETEkskul);
    ...
    const handleAdd = async ( judul: string, deskripsi: string, contentList: any[]) => {
        ...
        try {
            setIsLoading(true);
            const response = await axios.post(API_ENDPOINTS.TAMBAHEkskul, formData, {
                headers: {
